...
# check How does the development of patterns affect the average biomass in equilibrium
print(paste("Mean biomass :", mean(M)))

# D=0.0005 "Mean biomass : 37.5063049613844"
# D=0.005"Mean biomass : 411.245571744391"

# calculate How do the patterns affect the population processes, 
# such as the average food intake per mussel, or the average mortality of the mussels
Consumption = c/h*A*M
...
# show the biomass change over the time
MusselRec.df_Dsmall<-data.frame(timestamp=c(1:360), musselbiomass=MusselRec)
str(MusselRec.df_Dsmall)
p_Dsmall<-ggplot2::ggplot(aes(x=timestamp, y=musselbiomass), data=MusselRec.df) +
  geom_point()
p_Dsmall

MusselRec.df_Dlarge<-data.frame(timestamp=c(1:360), musselbiomass=MusselRec)
str(MusselRec.df_Dlarge)
p_Dlarge<-ggplot2::ggplot(aes(x=timestamp, y=musselbiomass), data=MusselRec.df) +
  geom_point()
p_Dlarge
